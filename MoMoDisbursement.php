...
class MoMoDisbursement
{
    ...
    public function transfer(
        ...
        // If request was accepted (202), return immediately
        if ($response['http_code'] === 202) {
            return [
                ...
                'status' => [
                    'data' => ['status' => 'PENDING'],
                    'message' => 'Transfer request sent. Check status.'
                ]
            ];
        }
    ...
    private function makeRequest($method, $url, $data = null, $headers = [], $waitForResponse = true)
    {
        ...
        if (!$waitForResponse && $method === 'POST') {
            $timeoutSettings[CURLOPT_TIMEOUT] = 45;
        }
        ...
        if ($error) {
            if ($method === 'POST' && strpos($error, 'Timed out') !== false) {
                error_log("Transfer request timed out - transfer may have succeeded, check status");
                return [
                    'http_code' => 202,
                    'data' => null,
                    'raw_response' => '',
